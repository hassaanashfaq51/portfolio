export const initialProjects = [
  {
    id: "proj-suroor",
    title: "SUROOR — Music Streaming Platform",
    description: "A modern music streaming platform built for discovering, organizing, and enjoying music through a smooth and responsive interface.",
    technologies: [
      "React.js",
      "Supabase",
      "JavaScript"
    ],
    features: [
      "Music playback and audio controls",
      "Search functionality",
      "Music categories",
      "Pakistani, Indian, and international music",
      "Playlists",
      "Favorite tracks",
      "Recently played music",
      "User authentication",
      "User profile functionality",
      "Supabase backend integration",
      "Responsive design",
      "Premium and smooth UI/UX"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/Music-Player",
    image_url: "/assets/project-suroor.webp",
    alt_text: "SUROOR music streaming platform",
    created_at: "2026-09-20T00:00:00.000Z"
  },
  {
    id: "proj-visionix",
    title: "VISIONIX — Social Networking Platform",
    description: "A modern social networking platform for connecting with people, sharing content, and interacting through real-time social features.",
    technologies: [
      "React.js",
      "Supabase",
      "JavaScript"
    ],
    features: [
      "User authentication",
      "User profiles",
      "Create and view posts",
      "Likes and comments",
      "Friend requests",
      "Connection management",
      "Real-time social interactions",
      "Real-time notifications",
      "Multimedia content",
      "Privacy controls",
      "Supabase backend",
      "PostgreSQL database",
      "Responsive interface",
      "Modern social networking UI"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/visionix",
    image_url: "/assets/project-visionix.webp",
    alt_text: "VISIONIX social networking platform",
    created_at: "2026-09-18T00:00:00.000Z"
  },
  {
    id: "proj-1",
    title: "U2 Collective HR Management Portal",
    description: "A centralized full-stack HR management platform designed to streamline employee management, attendance tracking, task management, performance reviews, documents, and other daily HR operations.",
    technologies: [
      "React.js",
      "Node.js",
      "Express.js",
      "Supabase"
    ],
    features: [
      "Role-based authentication and access control",
      "CEO/Admin and Employee dashboards",
      "Employee management",
      "Automated attendance tracking through employee login and logout",
      "Task assignment and progress management",
      "Employee performance reviews",
      "Company announcements and notifications",
      "Employee document management",
      "Dashboard analytics and HR statistics",
      "Report generation and data export",
      "Restricted employee access to protect official company information"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/u2-collective-portal",
    image_url: "/assets/project-hrportal.webp",
    created_at: "2026-07-11T00:00:00.000Z"
  },
  {
    id: "proj-5",
    title: "TaskPilot — Modern Productivity Web App",
    description: "A modern and responsive task management application designed to help users organize tasks, manage priorities, track deadlines, and monitor progress through a clean interactive interface.",
    technologies: [
      "HTML5",
      "CSS3",
      "JavaScript",
      "Local Storage",
      "Responsive Design"
    ],
    features: [
      "Create new tasks",
      "Edit existing tasks",
      "Delete tasks",
      "Mark tasks as completed",
      "Organize tasks by priority",
      "Organize tasks by category",
      "Set and track due dates",
      "Automatically identify Upcoming tasks",
      "Automatically identify Due Today tasks",
      "Automatically identify Overdue tasks",
      "Search tasks instantly",
      "Filter tasks",
      "Sort tasks",
      "Track total tasks",
      "Track pending tasks",
      "Track completed tasks",
      "Track completion progress",
      "Light mode",
      "Dark mode",
      "Local Storage persistence",
      "Import task data",
      "Export task data",
      "Keyboard shortcuts",
      "Responsive desktop layout",
      "Responsive tablet layout",
      "Responsive mobile layout",
      "Smooth animations",
      "Polished user interactions"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/Task-pilot",
    image_url: "/assets/project-taskpilot.webp",
    created_at: "2026-07-10T12:00:00.000Z"
  },
  {
    id: "proj-2",
    title: "User Management API — MongoDB Integration",
    description: "A database-integrated RESTful API for managing users with MongoDB, Mongoose, schema validation, persistent storage, and complete CRUD operations.",
    technologies: [
      "Node.js",
      "Express.js",
      "MongoDB",
      "REST API",
      "Postman"
    ],
    features: [
      "MongoDB Atlas database integration",
      "Mongoose schema design",
      "Persistent cloud database storage",
      "Create user functionality",
      "Retrieve all users",
      "Retrieve a single user by ID",
      "Update user information",
      "Delete users",
      "Complete CRUD operations",
      "Name validation",
      "Email validation",
      "Unique email constraint",
      "Age validation",
      "Environment variable configuration",
      "Centralized error handling",
      "Invalid MongoDB ObjectId handling",
      "Duplicate email handling",
      "RESTful API architecture",
      "API testing with Postman"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/decodelab-task3",
    image_url: "/assets/project-mongodb.webp",
    created_at: "2026-07-10T00:00:00.000Z"
  },
  {
    id: "proj-3",
    title: "RESTful Backend API",
    description: "A structured backend API built with Node.js and Express.js, demonstrating RESTful routing, request handling, middleware, and backend development fundamentals.",
    technologies: [
      "Node.js",
      "Express.js",
      "JavaScript",
      "REST API",
      "Postman"
    ],
    features: [
      "RESTful API architecture",
      "Structured API routes",
      "HTTP request handling",
      "GET, POST, PUT, and DELETE operations",
      "Middleware integration",
      "Request validation",
      "Structured JSON responses",
      "Error handling",
      "API testing with Postman"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/decodelab-task2",
    image_url: "/assets/project-backend.webp",
    created_at: "2026-07-09T00:00:00.000Z"
  },
  {
    id: "proj-4",
    title: "Responsive Frontend Interface",
    description: "A responsive and mobile-first frontend interface focused on clean layouts, modern styling, and a smooth user experience across different screen sizes.",
    technologies: [
      "HTML5",
      "CSS3",
      "JavaScript (Vanilla)",
      "CSS Grid",
      "Responsive Design"
    ],
    features: [
      "Responsive layout",
      "Mobile-first approach",
      "Structured HTML5 markup",
      "Modern CSS styling",
      "CSS Grid-based layouts",
      "Adaptive design for desktop, tablet, and mobile",
      "Clean user interface",
      "Organized and maintainable frontend structure"
    ],
    live_url: null,
    github_url: "https://github.com/hassaanashfaq51/decodelab-task1",
    image_url: "/assets/project-frontend.webp",
    created_at: "2026-07-08T00:00:00.000Z"
  }
];

export default initialProjects;
